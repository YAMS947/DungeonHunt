    ...
    while True:
        ...
        if userName in data["users"]:
            while True:
                print("Ingrese su contraseña")
                userPassword = str(input())
                if userPassword in data["users"][userName][userPassword]:
                    global equipment
                    global statistics
                    global inventory
                    equipment = data["users"][userName]["equipement"]
                    statistics = data["users"][userName]
                    inventory = data["users"][userName]["inventory"]
                    #Llamar un objeto desde el inventario: objects[inventory[Posicion en el indice][0]][inventory[Posición en el indice][1]]
                    break
...
gather_("weapon","woodenSword")
gather_("head","woodenHelmet")
start_Menu()
>>>>>>> f5f3e04c9bb9524c7b72794b4e226e9c85ac00d4
